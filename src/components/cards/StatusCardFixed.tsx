...
interface StatusCardProps {
    ...
    onSendEmail?: (selectedIds: number[]) => Promise<void>;
}
...
const emailFilterOptions = [
    ...
    { value: 'AC', label: 'Recepción de Actualización por Colaborador' },
    { value: 'EC', label: 'Proceso de Correo' },
    { value: 'ER', label: 'Rechazado de Correo' }
];
...
export default function StatusCard({ 
    ...
    const [emailFilter, setEmailFilter] = useState('EC');
    ...
    const shouldShowBulkActions = useMemo(() => {
        ...
        // Show bulk actions only for AC filter
    ...
    const filteredItems = useMemo(() => {
        ...
        if (showEmailFilter && emailFilter !== 'all') {
            filtered = filtered.filter(item => item.status === emailFilter);
        }
        
        // When 'Todos' is selected and bulk actions are shown, only show AC items for selection
        if (showEmailFilter && emailFilter === 'all' && shouldShowBulkActions) {
            filtered = filtered.filter(item => item.status === 'AC');
        }
        ...
        return filtered;
    }, [items, searchTerm, emailFilter, showEmailFilter, shouldShowBulkActions]);
    ...
    const handleSendEmail = async () => {
        if (!onSendEmail || selectedItems.size === 0) return;
        
        setIsSendingEmail(true);
        try {
            await onSendEmail(Array.from(selectedItems));
            setSelectedItems(new Set());
    ...
    return (
        <>
                ...
                <CardContent sx={{ p: 3 }}>
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                            {shouldShowBulkActions && expanded && selectedItems.size > 0 && (
                                <Typography 
                                    variant="caption" 
                                    sx={{ 
                                        color: color,
                            ...
                            {showEmailFilter && (
                                <Box sx={{ mb: 2 }}>
                                    <FormControl fullWidth size="small">
                                        <InputLabel id="email-filter-label">Filtrar por estado de correo</InputLabel>
                                        <Select
                                            labelId="email-filter-label"
                                            value={emailFilter}
                                            onChange={handleEmailFilterChange}
                                            label="Filtrar por estado de correo"
                                            sx={{ '& .MuiOutlinedInput-notchedOutline': { borderColor: `${color}40` } }}
                                        >
                                            ...
                                            {showEmailFilter && onSendEmail && (
                                                <Button
                                                    ...
                                                    sx={{ 
                                                        borderColor: color,
                                                        color: color,
                                                        '&:hover': { borderColor: `${color}dd`, bgcolor: `${color}10` },
                                                        minWidth: 'auto'
                                                    }}
                                                >
                                                    {isSendingEmail ? 'Enviando...' : `Email (${selectedItems.size})`}
                                                </Button>
                                            )}
                                            <Button
                                                size="small"
                                ...
                                {paginatedItems.length > 0 ? (
                                    paginatedItems.map((item, index) => (
                                        ...
                                        >
                                            ...
                                            <ListItemIcon sx={{ minWidth: shouldShowBulkActions ? 40 : 'auto' }}>
                                                <Avatar 
                                                    sx={{ 
                                                        width: 32, 
                                                        height: 32,
                                                ...
                                                secondary={
                                                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5, mt: 0.5 }}>
                                                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                                                            ...
                                                            <Box sx={{ fontSize: '0.75rem', color, fontWeight: 600 }}>
            ...
            {/* Email Sending Confirmation Dialog */}
            <Dialog 
            ...
            >
                ...
                <DialogContent>
                    ...
                    <Typography variant="body2" color="text.secondary">
                        Se enviarán correos electrónicos a los contactos seleccionados.
                    </Typography>
                    {isSendingEmail && (
                        <Box sx={{ mt: 2 }}>
                            <LinearProgress />
